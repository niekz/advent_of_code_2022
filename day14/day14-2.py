...
def draw(scan):
    for i in range(0, max_i + 1): # i is top to bottom
        print(str(i) + "\t" + ''.join(scan[i][min_j - 100:max_j + 100])) # draw one line at a time, much faster than 1 character
...
while not voiding:
    ...
    while moving:
        ...
        # print("CHECKING : " + str(current_i) + "," + str(current_j))
        # print("CHECKING : " + grid[current_i][current_j])
        ...
        if can_move_down(grid, current_i, current_j):
            ...
            settled = settled + 1
            if current_i == 0 and current_j == 500:
                voiding = True
                # draw(grid)
            # draw(grid)
